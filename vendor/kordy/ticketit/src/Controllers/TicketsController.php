...
class TicketsController extends Controller
{
    ...
    public function data($complete = false)
    {
        ...
        $collection
            ...
            ->join('company', 'company.id', '=', 'ticketit.company_id')
        ...
        if (LaravelVersion::min('5.4')) {
            $collection->rawColumns(['subject', 'status', 'priority', 'category', 'agent','company']);
        }
    ...
    protected function PCS()
    {
        ...
        if (LaravelVersion::min('5.3.0')) {
            return [$companies->pluck('name','kode','id'), $priorities->pluck('name', 'id'), $categories->pluck('name', 'id'), $statuses->pluck('name', 'id')];
        } else {
            return [$companies->list('name','kode','id'), $priorities->lists('name', 'id'), $categories->lists('name', 'id'), $statuses->lists('name', 'id')];
        }
    }
    ...
    {
        list($companies, $priorities, $categories) = $this->PCS();

        return view('ticketit::tickets.create', compact('companies','priorities', 'categories'));
    }
